class Ball {
  ...
  Boolean ballXGoal;
  //
  //int ballCount = 5; //requires pure jave class for true static variables 
  ...
  //
  void gameStart() { //Completes the Ball Constructor, should be combined with speed 
    while ( directionX == 0 ) {
  ...
  void gamePlay() {
    ...
    if (ballX < (width * 0) + ballDiameter || ballX > width - ballDiameter) {
      ballXGoal = true; 
